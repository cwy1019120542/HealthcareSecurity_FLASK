    ...
    def clean_get_response(self):
        ...
        for data_group in self.response_data:
            for key in self.decimal_field_list:
                data_group[key] = self.to_float(data_group[key])
            data_group['attribute'] = self.merge_attribute(data_group)
            self.fill_field(data_group, ('id', 'settle_id', 'cure_id', 'self_number', 'person_type', 'hospital_id', 'hospital_name', 'hospital_level', 'hospital_place', 'start_date', 'end_date', 'settle_date', 'evidence_type', 'illness_name', 'cure_type', 'overall_percent', 'is_centre', 'operator', 'town', 'village', 'remark'))
...
class SettleDataListDownload(SettleDataList):
    ...
    def clean_get_response(self):
        ...
        self.extra_response_data = ['序号', 'id', '结算ID', '就诊ID', '个人编号', '人员姓名', '证件号码', '人员类别', '定点医药机构编号', '定点医药机构名称', '医院等级', '医药机构地点类别', '开始日期', '结束日期', '结算日期', '就诊凭证类型', '总费用', '全自费金额', '超限价自费费用', '先行自付金额', '范围内费用', '起付线', '统筹基金支出', '大额医疗支出金额', '大病保险支出', '医疗救助支出', '公务员医疗补助', '其他基金支付', '基金支付总额', '个人现金支付', '个人账户支付', '账户共济支付金额', '病种名称', '医疗类别', '统筹基金支付比例', '中心报销', '经办人员', '乡镇', '村', '备注', '手机号', '人员属性']
