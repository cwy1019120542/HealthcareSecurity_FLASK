...
class SettleRate(Base):
    ...
    allowed_parameter = {
        "GET": {'year': ("enum", None, '', True, None), 'compare_year': ("enum", None, '', True, None),
            "person_type": ("enum", None, 'settle_data', False, None), "cure_type": ("enum", None, 'settle_data', False, None), "settle_date": ("combine_date", None, 'settle_data', False, None), "compare_settle_date": ("combine_date", None, 'settle_data', False, None),
    ...
    def make_get_query(self):
        compare_year = self.parameter_dict.pop('compare_year')
        settle_date_dict = {'year': self.parameter_dict[self.model_name].pop('settle_date', None), 'compare_year': self.parameter_dict[self.model_name].pop('compare_settle_date', None)}
        self.extra_response_data = {'year': self.year, 'compare_year': compare_year}
        for year_key, year in self.extra_response_data.items():
            settle_data = settle_date_dict[year_key]
            if settle_data:
                self.parameter_dict[self.model_name]['settle_date'] = settle_data
            else:
                self.parameter_dict[self.model_name].pop('settle_date', None)
    ...
    def clean_get_response(self):
        ...
        for hospital_place in EnumerateData.hospital_place:
            for year in ('compare_year', 'year'):
                for is_centre in (0, 1):
                    data_group_dict.setdefault(hospital_place, {}).setdefault(year, {}).setdefault(is_centre,{}).setdefault('hospital_place', hospital_place)
                    data_group_dict[hospital_place][year][is_centre]['year'] = self.extra_response_data[year]
                    data_group_dict[hospital_place][year][is_centre]['settle_type'] = settle_type_dict[is_centre]
                    for field in self.sum_field_list:
                        data_group_dict[hospital_place][year][is_centre][field] = 0
        for query_group in self.query_list:
            ...
            for data in data_list:
                ...
                for field in self.sum_field_list:
                    data_group_dict[hospital_place][year][is_centre][field] = self.to_float(getattr(data, field))
        for hospital_place in EnumerateData.hospital_place:
            ...
            for year in ('compare_year', 'year'):
                ...
                for is_centre in (0, 1):
                    data_group = data_group_dict[hospital_place][year][is_centre]
                ...
                direct_data = data_group_dict[hospital_place][year][0]
                for field in self.sum_field_list:
                    percent = direct_data[field]/all_dict[field]
                    percent_dict[field] = self.to_percent(percent)
                    percent_group_dict[year][field] = percent
